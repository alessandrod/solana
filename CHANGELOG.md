...
* When XDP is enabled on a host with a bond or tunnel interface, you must pass in `--experimental-retransmit-xdp-interface <real-interface>` (e.g.: `eno17395np0`, not `bond0`). We must load the xdp program onto the physical interface. Otherwise, your host will try to attach the xdp program to the bonded/tunnel interface, which isn't a physical interface.
