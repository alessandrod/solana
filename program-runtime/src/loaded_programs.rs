...
impl LoadedPrograms {
    ...
    pub fn replenish(
        ...
        if let Some((existing, entry_index)) = index.and_then(|index| {
            second_level
                .fork_versions
                .get(index)
                ...
                && existing.effective_slot == entry.effective_slot
            {
                if matches!(existing.program, LoadedProgramType::Unloaded(_)) {
                    ...
                    second_level.fork_versions.remove(entry_index);
        ...
        second_level.fork_versions.insert(
            index.unwrap_or(second_level.fork_versions.len()),
            entry.clone(),
        );
        (false, entry)
    }
