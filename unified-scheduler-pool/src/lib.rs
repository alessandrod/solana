        ...
        let handler_main_loop = || {
            ...
            move || {
                let arena = unsafe {
                    ...
                    disable_decay(arena).unwrap();
                    arena
                };
                unsafe {
                    bind_current_thread_to_arena(arena).unwrap();
                }
...
unsafe fn mallctl_read<T: Copy>(name: &CStr) -> std::result::Result<T, c_int> {
    let mut out: T = mem::zeroed();
    let mut out_len: size_t = mem::size_of::<T>() as size_t;
...
unsafe fn mallctl_write<T>(name: &CStr, val: &T) -> std::result::Result<(), c_int> {
    let ret = je::mallctl(
        name.as_ptr(),
        ptr::null_mut(),
        ptr::null_mut(),
        (val as *const T).cast::<c_void>() as *mut c_void,
...
unsafe fn create_arena() -> std::result::Result<u32, c_int> {
    let name = CString::new("arenas.create").unwrap();

    let mut arena: u32 = 0;
    let mut arena_len: size_t = mem::size_of::<u32>() as size_t;

    let ret = je::mallctl(
        name.as_ptr(),
        (&mut arena as *mut u32).cast::<c_void>(),
        (&mut arena_len as *mut size_t),
        ptr::null_mut(),
        0,
    );
    if ret != 0 {
...
unsafe fn disable_decay(arena: u32) -> std::result::Result<(), c_int> {
    // -1 disables decay/purging for that arena.
    let disabled: ssize_t = -1;

    let dirty = CString::new(format!("arena.{}.dirty_decay_ms", arena)).unwrap();
    let muzzy = CString::new(format!("arena.{}.muzzy_decay_ms", arena)).unwrap();

    mallctl_write(dirty.as_c_str(), &disabled)?;
    mallctl_write(muzzy.as_c_str(), &disabled)?;

    // Optional sanity check.
    let d: ssize_t = mallctl_read(dirty.as_c_str())?;
    let m: ssize_t = mallctl_read(muzzy.as_c_str())?;
    assert_eq!(d, -1);
    assert_eq!(m, -1);
...
unsafe fn mallctl_cmd(name: &CString) -> std::result::Result<(), c_int> {
    let ret = je::mallctl(
        name.as_ptr(),
        ptr::null_mut(),
        ptr::null_mut(),
...
unsafe fn bind_current_thread_to_arena(arena: u32) -> std::result::Result<(), c_int> {
    let name = CString::new("thread.arena").unwrap();
    mallctl_write(name.as_c_str(), &arena)?;

    let flush = CString::new("thread.tcache.flush").unwrap();
    let _ = mallctl_cmd(&flush);

    // Prove it.
    let cur: u32 = mallctl_read(&name)?;
    assert_eq!(cur, arena);
    Ok(())
}
