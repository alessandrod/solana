...
impl SequentialFileReader<LargeBuffer> {
    ...
    pub fn new(file: File) -> io::Result<Self> {
        Self::with_capacity(DEFAULT_BUFFER_SIZE, file)
    }

    /// Create a new `SequentialFileReader` for the given `file` using internally allocated
    /// buffer of specified `buf_size` and default read size.
    pub fn with_capacity(buf_size: usize, file: File) -> io::Result<Self> {
        Self::with_buffer(file, LargeBuffer::new(buf_size), DEFAULT_READ_SIZE)
    }
}
...
impl<B: AsMut<[u8]>> SequentialFileReader<B> {
    ...
    pub fn with_buffer(file: File, mut buffer: B, read_capacity: usize) -> io::Result<Self> {
        ...
        Self::with_buffer_and_ring(buffer, ring, file, read_capacity)
    }
...
impl<'a> std::fmt::Debug for ReadBufState {
...
mod tests {
    ...
    fn check_reading_file(
        ...
        let mut reader = SequentialFileReader::with_buffer(file, buf, read_capacity)?;
